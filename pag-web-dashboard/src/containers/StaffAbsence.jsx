...
import { getStaffAttendance } from "../store/actions/staff";
import { getMonth } from "../helpers/monthHelper";
import { hmsToSeconds, secondsToHMS } from "../helpers/duration";
// import TableAbsence from "../components/TableAbsenceStaff";
...
export default function AbsenceReport() {
  ...
  const { absenceReport } = useSelector((state) => state.staff);
  const [filterReport, setFilterReport] = useState(absenceReport);
  ...
  let colTable = [
    ...
    },
    {
      ...
      cell: (row) => {
        return (
          <Link
            to={`/dashboard/staff/attendance/${row.id}`}
  ...
  useEffect(() => {
    dispatch(getStaffAttendance());
  }, [dispatch]);

  useEffect(() => {
    setFilterReport(absenceReport);
  }, [absenceReport]);

  function handleSearch(e) {
    const searchInput = e.target.value;
    const filteredReport = absenceReport.filter((val) => {
      // console.log(listStudent);
      if (!searchInput) {
        return val;
      } else if (
        val.Staff.fullname.toLowerCase().includes(searchInput.toLowerCase())
      ) {
        return val;
      } else {
        return false;
  ...
  function handleButtonFilter(e) {
    if (!inputMonth && !inputYear) {
      ...
      dispatch(getStaffAttendance({ month: inputMonth, year: inputYear }));
    }
  }
  function renderYear() {
    let option = [];
  ...
  return (
    <div className="w-full flex-grow p-6">
      <p className="text-xl pb-3 flex items-center">
        <i className="fas fa-calendar mr-3"></i> Monthly Staff Attendance Report
      </p>
      <div className="bg-white overflow-auto">
        {/* <TableAbsence report={absenceReport} search={searchInput} /> */}
        <DataTable
          noHeader
