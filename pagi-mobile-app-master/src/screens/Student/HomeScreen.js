...
import { checkIn, getTodayPresence } from '../../store/actions/absence';

export default function HomeScreen({ navigation }) {
  const dispatch = useDispatch();
  const { name, checkInTime, checkOutTime, profilePict } = useSelector(
    state => state.user,
  );
  ...
  useFocusEffect(
    useCallback(() => {
      dispatch(getTodayPresence('student'));
      return () => null;
    }, [dispatch]),
  );
        ...
        <View>
          {profilePict ? (
            <Thumbnail
              large
              source={{
                uri: `https://${profilePict.replace(/^https?:\/\//, '')}`,
              }}
            />
          ) : (
            <Thumbnail
